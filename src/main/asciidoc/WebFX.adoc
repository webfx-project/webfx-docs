...
In that process, the WebFX CLI takes over the cross-platform aspects: when a feature is platform-dependent (a different implementation exists for different platforms), the tool will pick up the right modules (the implementations matching the platform targeted by the executable modules).
...
WebFX offers a cross-platform way to start applications (as the entry point differs with GWT). The main class of a WebFX application is always `dev.webfx.platform.shared.services.boot.ApplicationBooter` which will correctly bootstrap your application whatever the targeted platform. It will find your JavaFX application (which has been automatically declared as a Java service by the WebFX CLI). The only difference between WebFX applications will be the module to execute.
