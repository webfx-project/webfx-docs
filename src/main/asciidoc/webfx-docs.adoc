...
[cols="1,^1,^1,^1,^1"]
|===
| Build platform | Build option |Target platform | Install command | Generated file(s)
...
Some builds require additional pieces of software to work. You can use `webfx install graalvm`, `webfx install wix`, etc... to install or upgrade them on your build machine. Also on macOS, for builds other than -gwt and -openjfx-fatjar, Xcode must be installed and configured for a proper code signing (otherwise the executables will refuse to run).

NOTE: The CLI can display help at different levels. For example, `webfx install --help` will display all available bump sub-commands.
