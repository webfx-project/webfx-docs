...
which can also be invoked by more simply by `webfx.sh` and `webfx.bat` script files.
...
Alternatively on Windows, you can just add the CLI repository path to the environment path.
...
NOTE: The CLI can display help at different levels. For example, `webfx bump --help` will display all available bump sub-commands, and `webfx bump cli --help` the usage of that specific command.
