...
In the next sections, we won't repeat all the commands, we will use only the last command to build and run the application in one go, but of course you can still choose to separate the build and the run.
...
You can use the following command to target the Desktop platform (JAR):
...
----
<1> These tools are used to generate the .deb and .rpm installers.
...
----
<1> Actually just check if Xcode is installed with a correct path, otherwise suggest solutions.
...
Then, you can use the following command to target the Desktop platform (embed JRE):
...
Then, you can use the below command to target the Desktop platform (native):
...
WARNING: On some Linux virtual machines that don't support OpenGL, JavaFX may report: Error initializing QuantumRenderer: no suitable pipeline found.
...
In this context, running the application means invoking the Gluon Maven plugin to install and run the generated package on your Android device connected to your Linux machine via USB. For this to succeed, you must first activate the Developer mode and then the USB debugging option on your Android device.
