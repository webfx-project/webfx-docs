...
WebFX is a JavaFX application transpiler powered by link:https://www.gwtproject.org[GWT^] or link:https://www.teavm.org[TeaVM^]. It can transpile a JavaFX application into a standalone, self-contained JavaScript or WebAssembly web app (with no plugin or server required for execution in the browser).
...
' Adding more hidden arrows to force vertical alignment of elements in the "Your repository" rectangle
app -[hidden]> gwt
app -[hidden]> gwt
gwt -[hidden]> wa
----

The link:https://github.com/webfx-project/webfx/tree/main/webfx-kit[WebFX kit^] is the heart of WebFX. It's a modified version of OpenJFX that can be transpiled by GWT and TeaVM. This is achieved by patching the higher layer of OpenJFX (which contains the main JavaFX features and API) to make it GWT and TeaVM compatible, and by replacing the lower layer (the graphic rendering pipeline) with a scene graph -> DOM mapper (rendered by the browser).

=== Benefits

==== No server

There are already great solutions to run Swing or JavaFX applications in the browser without plugins by actually running them on a server. Those solutions avoid some current WebFX limitations, but a standard self-contained web packaging is a much simpler, more scalable, and more reliable execution model. This is precisely the benefit of WebFX, and probably the main reason why you would prefer it over the other existing solutions.
...
In addition to the web platform, a WebFX application can also run on desktop and mobile. WebFX will invoke the standard Java toolchain to produce desktop executables (with an optimized JRE embedded), and the Gluon toolchain to produce native executables for desktop and mobile.

Some alternative technologies or JVM languages allow you to do the same but only for the application logic, because they don't offer a cross-platform UI toolkit. With WebFX, you can do full cross-platform development of your entire application from a single codebase.

==== Java full-stack

Writing your whole stack in Java is a big advantage, keeping your environment simple and homogeneous from a single Java IDE. Here, full-stack means you can use Java on both the frontend and backend. This prevents you from having to master other complex ecosystems such as JavaScript or TypeScript, and also allows you to share common code between your backend and frontend using the Java module system - a great advantage compared to heterogeneous systems.

==== Performance

Despite the large size of OpenJFX, WebFX can produce lightweight web apps, as demonstrated by the demos:

[cols="1,^1,^1,^1"]
|===
|WebFX demo | GWT JS size * | TeaVM JS size * | TeaVM Wasm size *

|Colorful circles
|link:https://colorfulcircles.webfx.dev[97 kB^]
|link:https://colorfulcircles-teavm.webfx.dev[176 kB^]
|link:https://colorfulcircles-wasm.webfx.dev[220 kB^]
|Particles
|link:https://particles.webfx.dev[97 kB^]
|link:https://particles-teavm.webfx.dev[181 kB^]
|link:https://particles-wasm.webfx.dev[226 kB^]
|Tally counter
|link:https://tallycounter.webfx.dev[113 kB^]
|link:https://tallycounter-teavm.webfx.dev[200 kB^]
|link:https://tallycounter-wasm.webfx.dev[253 kB^]
|DemoFX
|link:https://demofx.webfx.dev[147 kB^]
|link:https://demofx-teavm.webfx.dev[241 kB^]
|link:https://demofx-wasm.webfx.dev[303 kB^]
|Modern gauge
|link:https://moderngauge.webfx.dev[157 kB^]
|link:https://moderngauge-teavm.webfx.dev[225 kB^]
|link:https://moderngauge-wasm.webfx.dev[278 kB^]
|Mandelbrot
|link:https://mandelbrot.webfx.dev[159 kB^]
|link:https://mandelbrot-teavm.webfx.dev[247 kB^]
|link:https://mandelbrot-wasm.webfx.dev[310 kB^]
|Cube
|link:https://cube.webfx.dev[160 kB^]
|link:https://cube-teavm.webfx.dev[264 kB^]
|link:https://cube-wasm.webfx.dev[317 kB^]
|SpaceFX
|link:https://spacefx.webfx.dev[171 kB^]
|link:https://spacefx-teavm.webfx.dev[259 kB^]
|link:https://spacefx-wasm.webfx.dev[331 kB^]
|FX2048
|link:https://fx2048.webfx.dev[206 kB^]
|link:https://fx2048-teavm.webfx.dev[341 kB^]
|link:https://fx2048-wasm.webfx.dev[428 kB^]
|Enzo clocks
|link:https://enzoclocks.webfx.dev[296 kB^]
|link:https://enzoclocks-teavm.webfx.dev[371 kB^]
|link:https://enzoclocks-wasm.webfx.dev[484 kB^]
|===

[small]#* compressed JS/Wasm size transferred over the network, excluding images and other resources#
...
* GWT and TeaVM run a dead code elimination (tree-shaking) process, which removes the JavaFX classes not used by the WebFX application.
* GWT produces very compact and optimized JS code, leading to highly performant web apps.

==== Gradual learning curve

WebFX is not an additional UI toolkit to learn - it is an emulation of the well-known and well-documented JavaFX API. All the powerful JavaFX features that you know and love are available for your web app. You will feel right at home with WebFX!

==== Fast development cycles

You don't need to run regular GWT/TeaVM compilations as in traditional GWT/TeaVM development, because you can already run and debug your WebFX application directly in your Java IDE with the OpenJFX runtime. You typically transpile your app only at the end of a development cycle to check the web output, after you have finished developing a feature using the standard JavaFX development model.
...
The WebFX kit coverage is currently limited to the essential features of JavaFX. So to successfully compile to the web, your JavaFX code needs to meet these two requirements:

* use only the features covered by the WebFX kit (see the link:https://javadoc.webfx.dev[JavaDoc^] for coverage)
* be compatible with GWT and/or TeaVM (avoid reflection, multithreading, and blocking code)

When a JavaFX application meets these two requirements, we call it a _WebFX application_, and it can be transpiled for the web simply by running a GWT or TeaVM compilation of it together with the WebFX kit.

The main JavaFX features not currently covered are: JavaFX CSS, FXML, 3D, and most complex controls such as ComboBox, TableView, and TreeView. We plan to support CSS (using efficient browser CSS mapping rather than JavaFX CSS emulation) and provide simpler alternative WebFX controls to ComboBox and TableView before considering their actual emulation.

NOTE: OpenJFX is a huge library (about 10MB) compared to standard JS frameworks (typically 100KB). Completing its coverage will take time, but you can already do most of what you can do with other standard frameworks.
...
The WebFX CLI is an essential Command Line Interface tool that will assist you in developing WebFX applications. It will create your application modules as follows:
...
|_ xxx-application-teavm-js (5)
|_ xxx-application-teavm-wasm (6)
end legend
----
...
<3> This module targets the web platform (JavaScript) via GWT. It binds your application with the WebFX kit and the GWT JavaScript compiler.
<4> This module targets the standard desktop platform. It binds your application with the OpenJFX runtime, and is directly executable in your IDE. It can also call the standard Java toolchain to produce the desktop executables (Windows, macOS or Linux) with an embedded JRE.
<5> This module targets the web platform (JavaScript) via TeaVM. It binds your application with the WebFX kit and the TeaVM JavaScript compiler.
<6> This module targets the web platform (WebAssembly) via TeaVM. It binds your application with the WebFX kit and the TeaVM WebAssembly compiler.
...
This directive asks the CLI to identify the list of your dependencies from an analysis of your source code, and automatically populate the dependencies in pom.xml, module-info.java, module.gwt.xml, etc...

During that process, the CLI takes care of the cross-platform aspects: when a feature is platform-dependent (a different implementation exists for different platforms), it will pick the right modules (those whose implementation matches the target platform). It is at this point, for example, that the CLI will replace the OpenJFX modules with the WebFX kit modules in your GWT and TeaVM application modules.
...
This generates an executable fat jar in the target folder that you can execute with java:
...
To easily invoke the CLI from a terminal, we need to create a permanent _webfx_ alias. Run these commands in the webfx-cli directory:
...
You can check for updates at any time by running:
...
Let's create our first WebFX application. We need to create the repository directory and ask the CLI to initialize it, passing the groupId, artifactId, and version of our application.
...
The init command creates only two files: webfx.xml and pom.xml. Note that if this is the first time you have used the CLI, it will download some other files through Maven to retrieve essential information about the available WebFX modules.

==== Creating your application modules

When you create an application, you pass the fully qualified name of the JavaFX class you want to create, and the prefix to use for the application modules:

 webfx create application --prefix webfx-example org.example.webfxexample.WebFxExampleApplication --helloWorld

////
TIP: You can omit the prefix here, because the CLI takes the parent module name in that case.
////
...
|_ webfx-example-application-teavm-js
|_ webfx-example-application-teavm-wasm
end legend
----
...
To generate the executables for the different platforms, you need to pass some extra build options. You can find the list in the build help:
...
The `run` command has exactly the same options as the `build` command. The only difference between these two commands is that with `run` you can run the app without rebuilding it (unless `--build` is specified), and with `build` you can build the app without running it (unless `--run` is specified).

=== Web platform via TeaVM

The link:https://teavm.org/docs/tooling/maven.html[TeaVM Maven plugin^] will be invoked to generate the web application.
...
----
webfx build --teavm <1>
webfx build --teavm --locate <2>
webfx build --teavm --show <3>
webfx run --teavm --javascript <4>
webfx build --teavm --javascript --run <5>
----
<1> Build the HTML executable files (JavaScript and WebAssembly) with the TeaVM Maven plugin
<2> Locate the generated executable files
<3> Show the generated executable files in the file explorer
<4> Execute the generated JavaScript executable in the browser
<5> Build the JavaScript app and execute it in a single command
...
NOTE: `webfx run --teavm --wasm` (short syntax `webfx run -tw`) does not work because http(s) is required for running WebAssembly. But you can use the built-in web server of your Java IDE to serve the file located by `webfx build -twl`.

In the next sections, we won't repeat all these commands; we will use only the last command to build and run the application in one go, but you can still choose to separate build and run.

=== Desktop platform (JAR)

NOTE: The desktop JAR executable is usually used for local testing. If you want to distribute your desktop application, the embedded JRE or native executables are preferred, as they don't require Java installed on the users' machines.
...
=== Desktop platform (embedded JRE)

As opposed to the JAR, the embedded JRE will be specific to the target platform. Therefore, a Linux machine will build a Linux executable, a Mac will build a macOS executable, and a Windows machine will build a Windows executable.
...
----
<1> Just checks that Xcode is installed with the correct path, otherwise suggests solutions.
...
NOTE: The CLI will help you to customize the installers (application name, icon, etc...) in a future version. For now, we just use the default settings.

IMPORTANT: On macOS, you need to configure at least a Team ID in Xcode to make the application runnable on your machine. We will provide a guide for this later.
...
As with the embedded JRE, three different machines are required to target the Linux, macOS, and Windows executables.
...
----
<1> Required for the GluonFX Maven plugin
<2> Just checks that Xcode is installed with the correct path, otherwise suggests solutions.
...
WARNING: On machines that don't support OpenGL (such as some Linux virtual machines), JavaFX may report: Error initializing QuantumRenderer: no suitable pipeline found. We plan to provide an option later to include the JavaFX software pipeline as a backup, but this will increase the size of the executable.
...
NOTE: Only 64-bit architecture is supported (the installation will fail on 32-bit devices). Please check that your Android kernel version specifies `x64`.

=== iOS platform

A macOS machine is required to build the iOS executable.
...
Instructions for IntelliJ IDEA are provided below, which can be easily adapted to other Java IDEs.
...
The way to boot GWT, TeaVM and OpenJFX applications is different, but WebFX offers a cross-platform way to do it. For this reason, #the main class of a WebFX application is always [red monospace]+dev.webfx.platform.boot.ApplicationBooter+#. It will find your JavaFX application because it has been automatically declared as a Java service by the CLI.

[NOTE]
====
GWT normally doesn't support the Java service API, but WebFX does, because the CLI emulates it by generating a GWT super source. You can rely on this feature to declare and implement your own services. Your services can even have platform-dependent implementations. For example, a service can be a cross-platform UI API, with an OpenJFX implementation and a different GWT/TeaVM implementation using the JS library you want for your web app. We will document this later.
====
...
Unlike the OpenJFX version where the IDE can use its own Java build system, the GWT version can only be built through Maven. One way to build it is to open a terminal window in your IDE, and to type `webfx build --gwt`.
...
It can be a good idea to create a configuration by selecting `Modify Run Configuration` in the context menu of that `package` goal, and press OK in the dialog. You can then easily run this configuration to trigger the GWT build.
...
Let's do a first small change by replacing the Text with a Button. At first sight, it looks like an insignificant change, but actually it's not, because this involves a new dependency on the `javafx-controls` module (our application was only using the `javafx-graphics` module so far). To take advantage of the CLI, let's enter the fully qualified Button class name like this:
...
The CLI should make all necessary changes in the `pom.xml`, `module-info.java`, and `module.gwt.xml` files. After a few seconds, your IDE should automatically detect and apply these changes. Then it should recognize the Button class, and allow you to import it.
...
First, check that the code works as expected with the OpenJFX runtime by running the OpenJFX configuration. Then build and run the GWT/TeaVM version using one of the methods already explained.

Our application looks a bit sad? Maybe you can try link:https://github.com/webfx-project/webfx-example/blob/main/webfx-example-application/src/main/java/org/example/webfxexample/WebFxExampleApplication.java[this little code^] that adds a bit of color, animation, and effects?

Got more creative ideas? Now is the time to play with WebFX!
