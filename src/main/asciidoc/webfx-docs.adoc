...
The `run` command has exactly the same options as the `build` command. The main difference is that with `run` you can run the app without rebuilding it (unless `--build` is specified), and with `build` you can build the app without running it (unless `--run` is specified).
