...
IMPORTANT: Be sure that `java`, `mvn` and `git` are referenced in the environment path. The WebFX CLI will invoke these commands without specifying their full path.
...
For now, let's just build the Web and OpenJFX desktop apps (we will target the other platforms later in this guide):
