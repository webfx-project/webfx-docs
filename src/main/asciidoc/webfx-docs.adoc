...
The init command just creates a webfx.xml and a pom.xml file at the root level of your repository. If this is the first time you use the CLI, it will also download some webfx.xml files from the Maven repository to get some meta info about the essential WebFX modules, in order to complete this job.
...
Explanation: GWT has a different entry point than standard Java applications, but WebFX offers a single cross-platform entry point that will correctly bootstrap your application. #The main class of a WebFX application will always be [red monospace]+dev.webfx.platform.shared.services.boot.ApplicationBooter+ whatever the platform#. It will find your JavaFX application because it has been automatically declared as a Java service by the CLI.
...
If you run this configuration, it will build and run your WebFX application with the OpenJFX runtime. This is this configuration that you will use to develop and debug your application.
