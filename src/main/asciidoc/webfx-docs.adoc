...
NOTE: Note for the impatient: OpenJFX is a huge library (about 10MB) compared to standard JS frameworks (typically 100KB). It will take time to complete its coverage (some parts may not be possible). Thanks for your understanding.
...
NOTE: This is sp far the only command that uses `git` (a `git pull` of the webfx-cli repository). The CLI will not call `git` on your own repositories.
...
You can use the following commands to target the Desktop platform with a fat JAR (you can still use the `--reveal`, `--locate` options in addition):
