...
NOTE: Some builds require additional third-party software to work. We will cover this later in this guide.

For now, let's just build the #OpenJFX fat jar# and #GWT html# executables (they don't require additional software):
...
To locate the generated executable files, just add the --locate option to the previous command:
...
Once your application is built, you can run the executable file with the `webfx run` command using the same syntax. So to run the OpenJFX fat jar, it will be:
...
And for the GWT html file, it will be:
...
We already covered how to generate the OpenJFX fat jar and GWT html executables. We will now cover the other executables.
...
Pre-requisites: you can install the following tools if you wish to also generate the installers:
...
<1> Optional. Inno Setup is used to create a more elaborate .exe installer.
...
Just replace `build` with `run` to run the generated executable.
...
Pre-requisites: you must install the following tools for a successful build:
...
webfx install ubuntu-devtools <1>
----
<1> Always required for the Gluon toolchain
...
Just replace `build` with `run` to run the generated executable. This will invoke the Gluon Maven plugin to install the executable on your Android device connected to your Linux machine via USB.
...
Just replace `build` with `run` to run the generated executable. This will invoke the Gluon Maven plugin that will install the executable on your iOS device connected to your Mac via USB.
