...
To do a simple build of your application without generating any executable, just type the following command:

 webfx build

To generate the executables you wish, you need to pass some extra build options. You can discover them in the build help:

 webfx build --help

Some builds require additional third-party software to work. The CLI will help you to install them through the `webfx install` command.
...
For now, let's just build the fat jar and html executables (we will target the other platforms later in this guide):

[source,shell,indent=0,role="primary"]
.Long syntax
----
webfx build --openjfx-fatjar --gwt
----
...
To locate the executable files, just add the --locate option to the previous command:

[source,shell,indent=0,role="primary"]
.Long syntax
----
webfx build --openjfx-fatjar --gwt --locate
----
...
If you prefer to reveal them in your file browser, use the `--reveal` option instead:

[source,shell,indent=0,role="primary"]
.Long syntax
----
webfx build --openjfx-fatjar --gwt --reveal
----
...
Once your application is built, you can run the executable file using the `webfx run` command using the same syntax. So to run the fat jar it will be:
...
And for the html file it will be:
...
=== Building the executables on local machines

==== OpenJFX desktops (embed JRE)

A Linux machine is required to build the Linux executables, a Mac for the macOS executables, and a Windows machine for the Windows executables.

Pre-requisites: if you want to also generate the installers, you will need to install these optional tools:
...
----
These tools are required to generate the .deb and .rpm installers.
...
----
<1> WiX Toolset is used to create a simple .msi installer.
<1> Inno Setup is used to create a more elaborate .exe installer.
...
Pre-requisites: as we use the Gluon toolchain here, GraalVM must be installed on the machine, with eventually other tools:
...
webfx install wix <1>
----
<1> Optional. WiX Toolset is used to create a simple .msi installer.
...
A Linux machine is required to build the Android executables.

Pre-requisites: as we use the Gluon toolchain here, GraalVM must be installed on the machine with some other dev tools:
...
A Mac is required to build the iOS executables.

Pre-requisites: as we use the Gluon toolchain here, GraalVM must be installed on the machine:
...
=== Building the executables on GitHub servers
