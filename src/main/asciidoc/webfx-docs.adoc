...
As opposed to the OpenJFX version where the IDE can use its own build system, the GWT version can only be built through the Maven build system.

One way to build it is to open a terminal window in your IDE, and to type the `webfx build -gwt`.

Another way is to trigger the build in the Maven IDE window, but be sure you activated the gwt-compile Maven profile.

You can also create a configuration that triggers the Maven build.

Once built, you can bookmark the generated html file in your IDE, and ask your IDE to open it in the browser.

==== Making changes

Let's do a first little change by replacing the Text with a Button. At first sight, it looks an insignificant change, but actually it's not, because this involves a new dependency to the `javafx-controls` module (so far, our application was only using the `javafx-graphics` module). To take advantage of the CLI, let's first enter the fully qualified Button class name like this:
...
The IDE doesn't recognize this class yet, but let's ask the CLI to update the build chain:

 webfx update

The CLI should make all necessary changes in the Java, Maven, and GWT build chain. After your IDE detected and considered these changes, it should recognize the Button class, and allow you to import it. Then, let's add a simple code to react to the button:
...
You can first check the code is working as expected with the OpenJFX runtime by running the OpenJFX configuration.

Then you can check the Web version by running a GWT build.
